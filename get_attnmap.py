...
ldm_stable = StableDiffusionPipeline.from_pretrained("/data/0shared/yangling/zxc/stable-diffusion-v1-4/").to(device)
...
def view_images_attn_each_step(t, images, num_rows=1, offset_ratio=0.02):
    if type(images) is list:
        num_empty = len(images) % num_rows
    elif images.ndim == 4:
        num_empty = images.shape[0] % num_rows
    ...
    pil_img = Image.fromarray(image_)
    pil_img.save(f'output_attn_{t}.png')
    display(pil_img)
...
def show_cross_attention_each_step(t, attention_store: AttentionStore, res: int, from_where: List[str], select: int = 0):
    tokens = tokenizer.encode(prompts[select])
    decoder = tokenizer.decode
    attention_maps = aggregate_attention_each_step(attention_store, res, from_where, True, select)
    images = []
    for i in range(len(tokens)):
        ...
        images.append(image)
    view_images_attn_each_step(t, np.stack(images, axis=0))
...
prompts = ["A closeup shot of a beautiful teenage girl in a white dress wearing small silver earrings in the garden, under the soft morning light"]
